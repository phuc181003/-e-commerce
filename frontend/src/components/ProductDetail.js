      ...
      {product.map((item) => (
        <div className="grid max-w-screen-xl  px-4  pb-8 mx-auto lg:gap-8  ">
          ...
          <div>
            ...
            <div className="mt-4">
              {activeTab === 1 && (
                <div className="p-4 grid bg-gray-100">
                  <p
                   className={`leading-relaxed text-center ${isExpanded ? 'block' : ' line-clamp-2 bg-gradient-to-t from-[#d1cfcf] from-10% to-[#ffffff] to-90%'}`}
                    dangerouslySetInnerHTML={{ __html: item.parameter }} />
                  {!isExpanded && (
        <button onClick={toggleExpand} className=" justify-items-center mt-2 py-2 px-4 border rounded text-blue-500 focus:outline-none">
          Show more
        </button>
      )}
      {isExpanded && (
